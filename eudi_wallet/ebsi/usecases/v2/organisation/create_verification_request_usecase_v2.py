...
class CreateVerificationRequestUsecaseV2:
    def __init__(
        self,
    ...
    def execute(
    ...
    ) -> Tuple[str, VerificationRecordModel]:
        state = str(uuid.uuid4())
        iss = domain
        exp = int(time.time()) + 3600
        response_type = "vp_token"
        response_mode = "direct_post"
        client_id = f"{domain}/organisation/{organisation_id}/service/digital-wallet/openid"
        redirect_uri = f"{domain}/organisation/{organisation_id}/service/digital-wallet/openid/direct_post"
        scope = "openid"
        ...
        if requestByReference:
            encoded_params = urllib.parse.urlencode(
                {
                    "request_uri": f"{domain}/organisation/{organisation_id}/service/digital-wallet/openid/sdjwt/verification/history/{verification_record_id}",
                }
            )
        else:
            encoded_params = urllib.parse.urlencode(
